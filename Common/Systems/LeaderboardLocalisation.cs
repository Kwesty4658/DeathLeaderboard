...
internal sealed class LeaderboardLocalisation : ModSystem
{
    ...
    private static LocalizedText GetText(string key) => Language.GetText($"Mods.{nameof(DeathLeaderboard)}.DeathCauses.{key}");
    ...
    internal static string GetLocalisationKey(PlayerDeathReason damageSource)
    {
	    ...
	    if (damageSource.SourceProjectileLocalIndex != -1)
		    return Main.projectile[damageSource.SourceProjectileLocalIndex]
			    .GetGlobalProjectile<ProjectileParentGlobalProjectile>()
			    .Parent.ModNPC.DisplayName.Key;
